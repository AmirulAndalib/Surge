...
func TestToRuntimeConfig_Exhaustive(t *testing.T) {
	...
	runtimeConfig := settings.ToRuntimeConfig()

	// 1. Verify that every field in runtimeConfig is non-zero when populated on input
	v := reflect.ValueOf(*runtimeConfig)
	typeOfS := v.Type()
	...
	// 2. Perform reciprocal reflection checks to ensure all fields in types.RuntimeConfig exist in either Settings.Network or Settings.Performance.
	networkType := reflect.TypeOf(settings.Network)
	performanceType := reflect.TypeOf(settings.Performance)
	runtimeType := reflect.TypeOf(*runtimeConfig)

	for i := 0; i < runtimeType.NumField(); i++ {
		fieldName := runtimeType.Field(i).Name
		// Ensure the field exists in either NetworkSettings or PerformanceSettings
		_, foundInNetwork := networkType.FieldByName(fieldName)
		_, foundInPerformance := performanceType.FieldByName(fieldName)
		if !foundInNetwork && !foundInPerformance {
			t.Errorf("Field %q in types.RuntimeConfig has no matching field in NetworkSettings or PerformanceSettings", fieldName)
		}
	}

	// Conversely, check that all fields in NetworkSettings and PerformanceSettings that should be converted exist in types.RuntimeConfig.
	...
	for i := 0; i < networkType.NumField(); i++ {
		fieldName := networkType.Field(i).Name
		if ignoredNetworkFields[fieldName] {
			continue
		}
		if _, found := runtimeType.FieldByName(fieldName); !found {
			t.Errorf("Field %q in NetworkSettings has no matching field in types.RuntimeConfig", fieldName)
		}
	}
